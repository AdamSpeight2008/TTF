...
Namespace Global.TTF.Testing
  ...
  Public Module Testing
    <Extension()>
    Public Function TimeNVerify(Of TReturn) _
    (MethodUnderTest As [Delegate],
     args() As Object,
     Optional ReturnResult As Boolean = False,
     Optional TestAlso As Boolean = False,
     Optional ByVal WhatIsExpected As TReturn = Nothing) As Timings.Timing
      ...
      Catch ex As Exception
        ...
        Return CeateFailure(sw.Elapsed, ex)
      Finally
      ...
      If TestAlso AndAlso (WhatIsExpected IsNot Nothing) Then
        ...
        If ReturnResult Then Return CreateValidResult(Of TReturn)(sw.Elapsed, Verified, Results)
        ' Otherwise return a ValidTiming
        Return CreateValidTiming(sw.Elapsed, Verified)
      Else
        ...
        If ReturnResult Then Return CreateResultTiming(Of TReturn)(sw.Elapsed, Results)
        ' Otherwise just return the timing.
        Return CreateTiming(sw.Elapsed)
      End If
    End Function
    Private Function Is_T_a_U(Of T, U)(First As T, second As T) As Boolean
      Return {First, second}.All(Function(x As T) TypeOf x Is U)
    End Function
    <Extension()> Private Function VerifiedAgainst(Of T)(ByVal Result As T, ByVal Expected As T) As Boolean
    ...
    <Extension()> Friend Function Match(Of T)(first As IEnumerable, second As IEnumerable) As Boolean
      ...
      Return True
    End Function
    <Extension()> Private Function [Not](x As Boolean) As Boolean
      Return Not (x)
    End Function
    <Extension()> Private Function Nor(x As Boolean, y As Boolean) As Boolean
      Return Not (x) AndAlso Not (y)
    End Function
    <Extension()> Private Function IsEqualTo(Of T As IComparable)(x As T, y As T) As Boolean
